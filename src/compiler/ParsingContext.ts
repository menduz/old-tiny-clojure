...
export class ParsingContext {
  ...
  public modulesInContext = new Map<string, Nodes.Atom>();
   ...
   * @param moduleName
   */
  invalidateModule(_moduleName: string) {
   ...
   * @param name
   */
  getInternalName(_moduleName: string, _name: string): string {
    throw new Error('not implemented getInternalName');
   ...
   * @param content
   */
  getParsingPhaseForContent(moduleName: string, content: string) {
    if (this.modulesInContext.has(moduleName)) {
      return this.modulesInContext.get(moduleName)!;
    } else {
      const parsing = getAST(moduleName, content, this);
      this.modulesInContext.set(moduleName, parsing);
      return parsing;
    }
  }

  getPhase(moduleName: string, phase: PhaseFlags, debug = false): Nodes.Atom {
    if (!this.modulesInContext.has(moduleName)) {
  ...
  private getParsingPhaseForFile(fileName: string, moduleName: string) {
    if (this.modulesInContext.has(moduleName)) {
      ...
      return this.getParsingPhaseForContent(moduleName, content);
    }
  }
